   ...
   let fieldsWithReferncePAth =   _AEEEE?.map((_fields:any)=> {
    ...
    let  matchFiledArr = _fields?.key && item?.rest["Work item type"] && _fields.value?.length && _fields.value.map((_item:any)=> {
     ...
     let _workItems = fieldsWithReferncePAth.filter((currentWorkItem:any)=> {
      console.log(item?.rest?.["Work item type"],":963",currentWorkItem.key);
...
const generateRequestBody = allNodes?.map((node:any) => {
  ...
  return {parentKey: formattedParentWorkItem ?  formattedParentWorkItem : "parent" ,workItemBody:_workItemBody,sequenceId: parentId}
}).flat();
...
function buildTree(parentId:any) {
  const children = [];
  for (const item of generateRequestBody) {
